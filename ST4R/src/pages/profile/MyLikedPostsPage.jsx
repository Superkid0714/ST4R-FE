...
// 좋아요한 글 목록 조회
export const useMyLikedPosts = (options = {}) => {
  return useQuery({
    queryKey: ['myLikedPosts', options],
    queryFn: async () => {
      const token = validateAndCleanToken();
      if (!token) {
      ...
      console.log('좋아요한 글 목록 조회 요청:', options);
      ...
      try {
        const response = await apiInstance.get(
          `/my/likedBoards?${params.toString()}`
        );
        console.log('좋아요한 글 목록 조회 성공:', response.data);
      ...
      <div className="min-h-screen bg-black text-white">
        <div className="px-4 py-6">
          ...
          <div className="flex items-center gap-3 mb-6">
            <BackButton />
            <h1 className="text-xl font-medium">내가 좋아한 글</h1>
          </div>
