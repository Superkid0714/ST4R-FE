export function GroupCreateSuccessModal({ onClose }) {
  return (
    <div className="relative w-96 font-['Pretendard'] p-5 flex flex-col text-center gap-6 bg-[#1D1D1D] rounded-2xl">
      ...
      <div className="flex flex-col gap-2">
        <div className="text-xl font-medium text-white">모임 만들기 완료!</div>
        <div className="text-sm text-[#8F8F8F]">
          새로운 모임이 성공적으로 등록되었습니다.
        </div>
      </div>
      ...
      <button
        ...
        onClick={onClose}
      >
        <span className="font-medium">나의 모임으로 돌아가기</span>
      </button>
    </div>
  );
}
